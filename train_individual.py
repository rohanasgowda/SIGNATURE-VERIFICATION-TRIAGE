...
# --- CONFIGURATION ---
DATA_FOLDER = "my_individual_data"
MODEL_PREFIX = "individual"
...
if __name__ == "__main__":
    ...
    # Save paths for visualization later
    example_genuine_path = None
    example_forged_path = None

    # 1. LOAD GENUINE
    print(f"--- Processing {MODEL_PREFIX.upper()} Genuine Signatures ---")
    genuine_files = glob(f"{DATA_FOLDER}/genuine/*.jpg") + glob(f"{DATA_FOLDER}/genuine/*.jpeg") + glob(f"{DATA_FOLDER}/genuine/*.png")
    if not genuine_files: print(f"Warning: No genuine images found in {DATA_FOLDER}/genuine")
    ...
    # 2. LOAD FORGED
    print(f"\n--- Processing {MODEL_PREFIX.upper()} Forged Signatures ---")
    forged_files = glob(f"{DATA_FOLDER}/forged/*.jpg") + glob(f"{DATA_FOLDER}/forged/*.jpeg") + glob(f"{DATA_FOLDER}/forged/*.png")
    if not forged_files: print(f"Warning: No forged images found in {DATA_FOLDER}/forged")
    ...
    # ==========================================
    # VISUALIZATION SECTION (Generating Images)
    # ==========================================
    ...
    if example_genuine_path:
        ...
        print("Saved Plot 1: Preprocessing")
        ...
        print("Saved Plot 2: LBP Texture Map")
        ...
        print("Saved Plot 3: HOG Gradient Map")
    ...
    print("Saved Plot 4: PCA Variance")

    # --- PLOT 5: 2D Scatter Plot (Cluster Map) ---
    # We do a temporary 2D PCA just for this picture
    ...
    print("Saved Plot 6: Confusion Matrix")
    ...
    print(f"\nTraining Complete. 7 Image files saved in {os.getcwd()}")