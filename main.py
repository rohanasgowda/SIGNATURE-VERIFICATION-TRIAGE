...
def preprocess_image(image_path, target_size=(1000, 500)):
    """
    Loads, preprocesses, and binarizes a signature image.
    """
...
def extract_features(binary_image):
    """
    Extracts LBP and HOG features from a preprocessed binary image.
    """
...
if __name__ == "__main__":
    ...
    # --- 1. Define paths ---
    genuine_path = "data/genuine/*.png"
    forged_path = "data/forged/*.png"
    MODEL_PREFIX = "generic" # Prefix for saved files

    all_features = []
    all_labels = []
    
    # For visualization later: get one example image path
    example_genuine_path = None
    example_forged_path = None

    print("--- Processing Genuine Signatures ---")
    genuine_files = glob(genuine_path)
    if not genuine_files: print("Warning: No genuine files found in data/genuine")
    ...
    forged_files = glob(forged_path)
    if not forged_files: print("Warning: No forged files found in data/forged")
    ...
    if example_genuine_path:
        ...
        plt.title('Step 3a: LBP Texture Features (Ink Texture)')
        plt.axis('off')
        ...
        plt.title('Step 3b: HOG Features (Stroke Directions)')
        plt.axis('off')
        plt.savefig(f'{MODEL_PREFIX}_plot_3_hog_features.png')
        print(f"Saved {MODEL_PREFIX}_plot_3_hog_features.png")
    
    
    # --- 2. Create the final X and y arrays ---
    X = np.array(all_features)
    y = np.array(all_labels)
    
    print(f"\n--- Dataset Created: {X.shape} samples ---")
    ...
    # Visualizing the Generic data in 2D space
    ...
    plt.title('Step 5: Generic Dataset Clusters (Green=Genuine, Red=Forged)')
    ...
    # --- MODEL TRAINING ---
    print("\n--- Training Models ---")
    # We will only save and visualize the BEST model (XGBoost usually wins)
    # but we can print results for others if you like.
    ...
    print("\n" + "="*50)
    print("Saving the trained Generic model...")

    joblib.dump(scaler, 'scaler.joblib')
    joblib.dump(pca, 'pca.joblib')
    joblib.dump(model, 'model.joblib')
    
    print("Models saved successfully as 'scaler.joblib', 'pca.joblib', 'model.joblib'")
